...
// computer part
int RandominRange (int arrayDim){
    srand(time(0));
    int num= (rand() % (arrayDim));
    return num;
}
int evenrandomnumber (int gridDim){
    srand(time(0));
    int num = 2*(rand()% (((gridDim*2-1)+1)/2));
    return num ;
}
int oddrandomnumber (int gridDim){
    srand(time(0));
    int num = 2*(rand()% ((gridDim*2-1)/2))+1;
    return num ;
}
void randomline (int gridDim , char gameArray[][gridDim*2-1]  ){

    int x= RandominRange(gridDim);
    int y ;
    if(x%2==0){
         y = oddrandomnumber ;
    }
    ...
    if (gameArray[x][y]==-1){
        ...
        randomline( gameArray[gridDim*2-1][gridDim*2-1] , gridDim);
    }
}
...
void gameLoop(int gridDim, char gameArray[][gridDim*2-1], int movesArray[][MOVES_ARRAY_WIDTH]){
    ...
    while(1){
        ...
        if( players[0].num==1){
            joinDots(p1, p2, gridDim, gameArray);
        ...
        else if (players[0].num==2){
            randomline(gridDim,gameArray[gridDim*2-1][gridDim*2-1]);
            int newCount = checkBoxes(gridDim, gameArray, players[0]);
