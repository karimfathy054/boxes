    ...
    while(1){
        ...
        if(players[0].num==1){
            ...
            if(result=='y' || result=='Y'){
                save(game,movesArray,gridDim,gameArray);
            }
...
int main(){
    ...
    scanf("%d",&modeNum);
    if(modeNum==3){
        char gameArray[ gridDim*2-1][ gridDim*2-1];
        initGameArray( gridDim*2-1, gameArray);
        int movesArray[2*gridDim*(gridDim-1)][MOVES_ARRAY_WIDTH];
        loadGame(game,movesArray,gridDim,gameArray);
